...
<script>
  ...
  const transitionDuration=prefixStyle('transitionDuration')
    ...
    export default {
      ...
      computed:{
        ...
        //当网络出问题，或者歌曲加载不出来的时候，此时前进或者后退键都变为灰，表示不可用
        disableCls(){
      ...
      methods:{
        ...
        updateTime(e){
          //获取当前播放的时间，这个事件在audio中的音乐播放的时候自动会派发出来，是audio的自身的固有方法
        ...
        format(interval){
          ...
          const second=this._pad(interval % 60) //这个函数_pad是为了补零
  ...
  //当歌曲加载失败，网络错误
    ...
    /*   leave(el,done){
          console.log('leave')
          this.$refs.cdWrapper.style.transition='all o.4s'
          const {x,y,scale}=this._getPosAndScale()
          console.log({
            x,y,scale
          })
